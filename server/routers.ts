...
export const appRouter = router({
  issues: router({
    ...
    create: publicProcedure
      .input(issueInput)
      .mutation(({ input }) =>
        createIssue({ ...input, userId: workspaceUserId })
      ),
    delete: publicProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(async ({ input }) => {
        if (!(await deleteIssue(input.id, workspaceUserId)))
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Issue não encontrada.",
          });
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          const result = issueAnalysisSchema.parse(
            await aiProvider.analyzeIssue(issue)
          );
          ...
          throw new TRPCError({
            ...
            message: "Não foi possível analisar a issue agora.",
          });
        }
      }),
  }),
